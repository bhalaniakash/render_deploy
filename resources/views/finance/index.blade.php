...
<div class="animate-fade-in">
    ...
    <div class="glass-effect rounded-xl p-4 md:p-6 mb-6 card-hover">
        ...
        <form method="POST" action="{{ route('finances.store') }}" class="space-y-4" id="transaction-form">
            @csrf
            ...
            <div class="block md:hidden space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="method" class="w-full px-3 py-2 text-sm rounded-lg border border-gray-300 input-focus transition-all" required>
                ...
                <div>
                    <button type="submit" class="w-full btn-primary text-white py-2 px-4 rounded-lg font-semibold transition-all text-sm">
            ...
            <div class="hidden md:grid md:grid-cols-7 gap-3 items-end">
                ...
                <div>
                    <button type="submit" class="w-full btn-primary text-white py-2 px-3 rounded-lg font-semibold transition-all text-sm">
    ...
    <div class="glass-effect rounded-xl p-4 md:p-6 mb-6 card-hover">
        ...
        <form method="POST" action="{{ route('finances.opening') }}" class="space-y-3 md:space-y-0 md:flex md:items-end md:gap-4">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-3 gap-3 md:flex-1">
                ...
                <div class="md:flex md:items-end">
                    <button type="submit" class="w-full bg-purple-500 hover:bg-purple-600 text-white py-2 px-4 rounded-lg font-semibold transition-all text-sm">
                ...
                <table class="w-full min-w-full">
                    <thead>
                        <tr class="bg-gray-50/80">
                            ...
                            <th class="px-3 py-2 text-right text-xs font-semibold text-red-600 uppercase tracking-wider">Expense</th>
                            <th class="px-3 py-2 text-right text-xs font-semibold text-gray-700 uppercase tracking-wider">Cash Bal</th>
                            <th class="px-3 py-2 text-right text-xs font-semibold text-gray-700 uppercase tracking-wider">GPay Bal</th>
                            <th class="px-3 py-2 text-right text-xs font-semibold text-gray-700 uppercase tracking-wider">Total Bal</th>
                            <th class="px-3 py-2 text-right text-xs font-semibold text-gray-700 uppercase tracking-wider">Actions</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-200">
                        @foreach($finances as $f)
                            <tr class="finance-row table-row-hover transition-all duration-200 hover:bg-blue-50/50">
                                <td class="px-3 py-2 text-xs text-gray-700 whitespace-nowrap">{{ $f->date->format('d/m/Y') }}</td>
                                <td class="px-3 py-2 text-xs font-medium text-gray-900 max-w-[120px] truncate" title="{{ $f->description }}">
                                    {{ $f->description }}
                                </td>
                                ...
                                <td class="px-3 py-2 text-xs text-right text-gray-700 whitespace-nowrap">₹{{ number_format($f->cash_balance, 2) }}</td>
                                <td class="px-3 py-2 text-xs text-right text-gray-700 whitespace-nowrap">₹{{ number_format($f->gpay_balance, 2) }}</td>
                    ...
                    <tfoot class="bg-gray-50/80 font-semibold">
                        <tr>
                            ...
                            <td class="px-3 py-2 text-xs text-right text-green-600 whitespace-nowrap">₹{{ number_format($totalIncome, 2) }}</td>
                            <td class="px-3 py-2 text-xs text-right text-red-600 whitespace-nowrap">₹{{ number_format($totalExpense, 2) }}</td>
                            <td class="px-3 py-2 text-xs text-right text-gray-700 whitespace-nowrap">₹{{ number_format($finalCashBalance ?? 0, 2) }}</td>
                            <td class="px-3 py-2 text-xs text-right text-gray-700 whitespace-nowrap">₹{{ number_format($finalGpayBalance ?? 0, 2) }}</td>
                            <td class="px-3 py-2 text-xs text-right text-gray-900 whitespace-nowrap">₹{{ number_format($finalBalance, 2) }}</td>
                            <td></td>
                        </tr>
                    </tfoot>
                </table>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (form) {
            ...
            const expenseInput = form.querySelector('input[name="expense"]');
            ...
            function validateAmounts() {
                if (incomeInput && expenseInput) {
                    if (incomeInput.value && expenseInput.value) {
                        // If both are filled, clear expense
                        expenseInput.value = '';
                    }
                }
            }
            ...
            form.addEventListener('submit', function(e) {
                ...
                if ((!income || income <= 0) && (!expense || expense <= 0)) {
                    e.preventDefault();
                    showToast('Please enter an amount for Income or Expense', 'error');
                    return false;
                }
            });
        }
        ...
        function syncFormControls() {
            ...
            // Disable hidden inputs to prevent validation issues
        ...
        window.addEventListener('resize', function() {
            ...
            resizeTimer = setTimeout(syncFormControls, 150);
        });
    });
</script>
@endpush
